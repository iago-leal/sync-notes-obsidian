...
def has_contradictions_signal(text: str) -> bool:
    ...
    return bool(
        re.search(
            r"RF-\d+.*?\b(mas|porém|entretanto|however|but)\b", text, re.IGNORECASE | re.DOTALL
        )
    )
...
def score_clareza(text: str) -> DimensionScore:
    ...
    subjects = count_pattern(
        rf_section, r"\b(o sistema|o usuário|a plataforma|the system|the user)\b"
    )
...
def score_edge_cases(text: str) -> DimensionScore:
    ...
    if ec_count == 0:
        dim.issues.append(
            "❌ CRÍTICO: Nenhum edge case definido — a implementação não saberá como tratar erros"
        )
        dim.raw_score = 0
        return dim
    ...
    if ec_count >= 4:
        ...
        score += 3
        dim.issues.append(
            f"⚠️ Apenas {ec_count} edge case(s) — adicione casos de falha externa e inputs inválidos"
        )
    else:
        score += 1
    ...
    if has_behavior >= ec_count * 2:  # pelo menos trigger + comportamento
        ...
        dim.issues.append(
            "⚠️ Edge cases sem comportamento definido — são apenas perguntas, não spec"
        )
