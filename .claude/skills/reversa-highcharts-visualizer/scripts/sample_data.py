...
def sample_line(months: int = 12, series_count: int = 2) -> dict:
    ...
    for i in range(series_count):
        ...
        data = [
            round(base + random.gauss(0, 20) + j * random.uniform(-2, 5), 1) for j in range(months)
        ]
...
def sample_pie(items: int = 6) -> dict:
    ...
    total = sum(values)
    data = [
        {"name": n, "y": round(v / total * 100, 1)} for n, v in zip(names, values, strict=False)
    ]
    # Destacar o maior
    ...
    return {
        "series": [{"name": "Participação", "colorByPoint": True, "data": data}],
        "suggested_type": "pie",
    }
...
def sample_scatter(points: int = 50) -> dict:
    ...
    data_b = [
        [round(random.gauss(8, 1.5), 1), round(random.gauss(3, 1.5), 1)] for _ in range(points)
    ]
    return {
...
def sample_heatmap(rows: int = 7, cols: int = 12) -> dict:
    ...
    col_cats = ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"][
        :cols
    ]
    data = [[x, y, random.randint(0, 100)] for x in range(cols) for y in range(rows)]
    return {
        "xCategories": col_cats,
        "yCategories": row_cats,
        "series": [{"data": data}],
...
def sample_stock(days: int = 365) -> dict:
    ...
    return {
        "series": [{"type": "candlestick", "name": "ACME", "data": data}],
